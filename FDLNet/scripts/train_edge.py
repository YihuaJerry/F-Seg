...
def parse_args():
    ...
    parser.add_argument('--workers', '-j', type=int, default=32,
    ...
    parser.add_argument('--save_dir', default='/home/ma-user/work/ymxwork/NIPS/train/runs/ckpt',
                        help='Directory for saving checkpoint models')
    parser.add_argument('--save_epoch', type=int, default=20,
                        help='save model every checkpoint-epoch')
    parser.add_argument('--log_dir', default='/home/ma-user/work/ymxwork/NIPS/train/runs/logs/',
                        help='Directory for saving checkpoint models')
...
class Trainer(object):
    ...
    def train_edge(self):
        ...
        for iteration, (images, targets, edge, _) in enumerate(self.train_loader):
            ...
            pre_images=self.model(images)
            #pre_images=pre_images.permute(1, 0, 2, 3)
            # gray_masks = torch.mean(pre_images, dim=1)
            # print('pre_images:{}'.format(pre_images.shape))
            # print("pre_images type:{}".format(pre_images.dtype))
            # print("target type:{}".format(targets.dtype))
            #targets=targets.to(dtype=torch.float)
            loss_dict= self.criterion(pre_images,targets)
            # print("loss:{}".format(loss_dict))
            # print("loss_dict:{}".format(loss_dict['seg_loss']))
            # print(type(loss_dict['seg_loss']))
    ...
    def ms_val(self, iteration):
        ...
        for i, batch_data in enumerate(tqdm(self.val_loader, ascii= True)):
            ...
            with torch.no_grad():
                ...
                #scores = torch.zeros(1, 20, segSize[0], segSize[1]).to(self.device).detach()
                for image in img_resized_list:
                    image = image.to(self.device)
                    target = target.to(self.device)
                    #a, b = model(image)
                    print("image_val_shape:{}".format(image.shape))
                    print("target_val_shape:{}".format(target.shape))
                    a = model(image)
                    logits = a
                    logits = F.interpolate(logits, size=size,
                                           mode='bilinear', align_corners=True)
                    print('logits succeed')
                    scores += torch.softmax(logits, dim=1)
                    print('scores succeed')
            ...
            print("self.metric.update(scores, target) succeed")
...
def save_checkpoint(model, optimizer, args, iteration, mIoU, is_best=False):
    ...
    #mp:
    if not os.path.exists(directory):  # 如果目录不存在，创建目录
...
if __name__ == '__main__':
    ...
    args.save_pred = True
    if args.save_pred:
        outdir = '/home/ma-user/work/ymxwork/NIPS/train/runs/pred_pic/{}_{}_{}'.format(args.model, args.backbone, args.dataset)
